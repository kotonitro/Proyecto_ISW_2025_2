...
const logoPath = path.join(
  __dirname,
  "../../../frontend/src/images/logoUBB.png"
);
...
export function generateInformePdf(Informe) {
  return new Promise((resolve, reject) => {
    ...
    const fechaTexto = Informe.fechaInforme
      ? new Date(Informe.fechaInforme).toLocaleDateString("es-CL", {
          weekday: "long",
          year: "numeric",
          month: "long",
          day: "numeric",
    ...
    const nombreEncargado = Informe.encargados
      ? Informe.encargados.nombre
      : "Sin asignar";
    ...
    if (Informe.bicicleta) {
      ...
      const color = Informe.bicicleta.color
        ? `(${Informe.bicicleta.color})`
        : "";

      const infoBici =
        `${marca} ${modelo} ${color}`.trim() || "Datos incompletos";
    ...
    if (Informe.bicicletero) {
      const infoLugar =
        Informe.bicicletero.nombre ||
        `Bicicletero #${Informe.bicicletero.idBicicletero}`;
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc
        ...
        .fillColor("grey")
        .text(
          `Generado automáticamente el ${new Date().toLocaleString()} - Página ${
            i + 1
          } de ${range.count}`,
          50,
          pageHeight - 50,
