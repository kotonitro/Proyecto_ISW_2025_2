...
export async function handleCreateInforme(req, res) {
  ...
  if (informeData.fechaInforme) {
    ...
    const esFormatoChileno = /^\d{1,2}-\d{1,2}-\d{4}$/.test(
      informeData.fechaInforme
    );
  ...
  try {
    ...
    if (!encargadoExiste) {
      return handleErrorClient(
        res,
        404,
        "El Encargado indicado no existe en el sistema."
      );
    }
    if (value.idBicicleta) {
      ...
      if (!bicicletaExiste) {
        return handleErrorClient(
          res,
          404,
          `La Bicicleta con ID ${value.idBicicleta} no existe.`
        );
      }
    }
    ...
    if (value.idBicicletero) {
      ...
      if (!bicicleteroExiste) {
        return handleErrorClient(
          res,
          404,
          `El Bicicletero con ID ${value.idBicicletero} no existe.`
        );
      }
    }
  ...
  try {
    ...
    if (!Informe) {
      return handleErrorClient(
        res,
        404,
        "Informe no encontrado para generar PDF."
      );
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=informe_${idInforme}.pdf`
    );
