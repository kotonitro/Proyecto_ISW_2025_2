...
export const bicicleteroValidation = Joi.object({
  ...
  ubicacion: Joi.string().trim().max(255).required().messages({
  ...
  imagen: Joi.string().optional().allow(null, '').messages({
    "string.base": "La imagen debe ser un nombre de archivo (texto).",
  }),
})
  .required()
  .messages({ "any.required": "La petición no puede estar vacia." });

export const bicicleteroUpdateValidation = bicicleteroValidation
