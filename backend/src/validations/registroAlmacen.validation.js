...
export const registroEntradaValidation = Joi.object({
  ...
  rutUsuario: Joi.string().trim().pattern(rutPattern).required().messages({
    "string.pattern.base": "RUT debe tener formato 12345678-K",
    "any.required": "RUT es obligatorio",
  }),

  nombreUsuario: Joi.string().trim().max(255).required().messages({
  ...
  telefonoUsuario: Joi.number().integer().positive().required().messages({
  ...
  idBicicleta: Joi.number().integer().positive().required().messages({
  ...
  idBicicletero: Joi.number().integer().positive().required().messages({
  ...
  fechaEntrada: Joi.date().iso().optional().messages({
    "date.base": "Fecha de entrada debe ser una fecha válida",
    "date.format":
      "Fecha de entrada debe estar en formato (YYYY-MM-DDTHH:mm:ss.sssZ)",
  }),

  fechaSalida: Joi.date().iso().optional().messages({
    "date.base": "Fecha de salida debe ser una fecha válida",
    "date.format":
      "Fecha de salida debe estar en formato (YYYY-MM-DDTHH:mm:ss.sssZ)",
  }),
});

export const registroSalidaValidation = Joi.object({
  idRegistroAlmacen: Joi.number().integer().positive().required().messages({
  ...
  fechaSalida: Joi.date().iso().optional().messages({
    "date.base": "Fecha de salida debe ser una fecha válida",
    "date.format":
      "Fecha de salida debe estar en formato ISO 8601 (YYYY-MM-DDTHH:mm:ss.sssZ)",
  }),
});
