...
const InformesList = ({ informes, onDescargar, onDescargarZIP }) => {
  ...
  const informesOrdenados = [...informes].sort(
    (a, b) => b.idInforme - a.idInforme
  );
  const indiceUltimoItem = paginaActual * itemsPorPagina;
  const indicePrimerItem = indiceUltimoItem - itemsPorPagina;
  const informesActuales = informesOrdenados.slice(
    indicePrimerItem,
    indiceUltimoItem
  );
            ...
            <table className="min-w-full border-collapse">
              <thead className="bg-gray-800 text-white">
                <tr>
                  <th
                    scope="col"
                    className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider border border-gray-600"
                  >
                    ID
                  </th>
                  <th
