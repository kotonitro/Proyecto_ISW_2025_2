...
export default function CustodiaForm({ onSuccess, addAlert }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      addAlert("error", e.message || "Error de validación en el servidor.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <form
      onSubmit={submit}
      className="bg-white p-5 rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-blue-50/50"
    >
      <h2 className="text-lg font-bold text-blue-900 mb-5 px-1">
        Registro de Entrada
      </h2>
          ...
          <div className="relative z-10">
            ...
            <div
              className={`transition-all duration-500 ease-in-out overflow-hidden ${
                userData
                  ? "max-h-96 opacity-100 pt-2 mt-2 border-t border-blue-100/80"
                  : "max-h-0 opacity-0"
              }`}
            >
              {userData && (
                <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  <div className="flex justify-between items-start bg-blue-50/50 p-2.5 rounded-lg border border-blue-100/50">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-blue-800 mb-0.5">
                        Nombre
                      </p>
                      <p className="text-sm font-bold text-blue-900 leading-tight line-clamp-1">
                        {userData.nombre}
                      </p>
                    </div>
                    <div className="flex flex-col items-center pl-2 border-l border-blue-200/40 ml-2">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-blue-800 mb-0.5 text-center">
                        Bicicletas registradas
                      </p>
                      <span className="text-xs font-bold text-blue-900">
                        {userData.bicicletas?.length || 0}
                      </span>
                    </div>
                  </div>
                  ...
                  {userData.bicicletas && userData.bicicletas.length > 0 && (
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setIdBicicletaSeleccionada(e.target.value)
                        }
                      ...
                      >
                        ...
                        {userData.bicicletas.map((bici) => (
                          <option
                            key={bici.idBicicleta}
                            value={bici.idBicicleta}
