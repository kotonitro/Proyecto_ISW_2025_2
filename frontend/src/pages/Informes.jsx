...
const Informes = () => {
  ...
  const informesFiltrados = informes.filter((informe) => {
    ...
    const fechaFormateada = new Date(informe.fechaInforme).toLocaleDateString(
      "es-CL",
      {
        timeZone: "UTC",
        year: "numeric",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.idBicicleta)
        data.append("idBicicleta", formData.idBicicleta);
      if (formData.idBicicletero)
        data.append("idBicicletero", formData.idBicicletero);
      ...
      const msg =
        error.response?.data?.message || "Hubo un error al crear el informe.";
      addAlert("error", msg);
    } finally {
      setLoading(false);
    }
  ...
  const mostrarSelectorBicicleta = [
    "Robo",
    "Daño Fisico",
    "Perdida",
    "ROBO",
  ...
  const mostrarSelectorBicicletero = [
    "Mantenimiento",
    "MANTENIMIENTO",
    "Robo",
    "Daño Fisico",
                  ...
                  >
                    ...
                    {listaBicicletas.map((bici) => (
                      <option key={bici.idBicicleta} value={bici.idBicicleta}>
                        #{bici.idBicicleta} - {bici.marca} {bici.modelo} (
                        {bici.color})
                      </option>
                    ))}
                  </select>
              ...
              {mostrarSelectorBicicletero && (
                <div className="md:col-span-2 bg-blue-50 p-4 rounded-lg border border-blue-100 animate-fadeIn">
                  <label className="block text-sm font-bold text-blue-800 mb-2">
                    {tipoActual &&
                    tipoActual.toUpperCase().includes("MANTENIMIENTO")
                  ...
                  >
                    ...
                    {listaBicicleteros.map((lugar) => (
                      <option
                        key={lugar.idBicicletero}
                        value={lugar.idBicicletero}
                      >
